#!/bin/sh
# Fuseo の配布・LP 計測を一目で表示する（プライバシー: クライアント側トラッキング無し）。
#   ./scripts/stats.sh
# - GitHub Release の実ダウンロード数（GitHub が標準集計。API は公開リポジトリなら認証不要）
# - LP(fuseo.ati-mirai.co.jp) の表示数・/dl クリック数（VPS の nginx ログをサーバ側集計）
REPO="supergodak/fuseo"
...
echo
echo "=== LP  https://fuseo.ati-mirai.co.jp ==="
ssh -o BatchMode=yes -o ConnectTimeout=12 sakura '
LOG=/var/log/nginx/fuseo.access.log
...
'
echo
echo "（詳細ダッシュボードが欲しい場合は VPS で: goaccess /var/log/nginx/fuseo.access.log）"
