...
- **4方向（0/90/180/270°）すべてを正立スコアで比較**する（2026-07-08改定）。旧仕様の「縦長=横倒しカード
  とみなす2択」は縦長の紙書類（住民票等）を強制横倒しにするバグがあった。4方向比較なら縦書類は縦のまま勝つ。
  正立スコア全ゼロ時は総量スコアにフォールバック。コストはOCR×4（+0.3〜0.5s/枚・許容）。
- 自動が外れた場合の救済として `analyze(manualRotation:)`（時計回り90°×n・正立化結果へ追加適用・再OCR）。
