...
/// 受理タイプ以外はその場でアラートし、読み込まない。PDF は WP-10 でページ画像に展開して受理する。
    ...
    private var dropZone: some View {
        VStack(spacing: 20) {
            ...
            Text("本人確認書類の写真をここにドロップ")
                .font(.title3)
            Text("対応形式: JPEG / PNG / HEIC / TIFF / PDF（フォルダごとでもOK）")
                .font(.callout)
                .foregroundStyle(.secondary)
        ...
        .alert("対応していない形式です", isPresented: $showingUnsupported) {
            ...
            Text("JPEG / PNG / HEIC / TIFF の画像、または PDF を選んでください。")
        }
    }

    private func handleDropped(_ urls: [URL]) {
        // フォルダは中の対応ファイルへ展開（一括処理 v1.2）
        let accepted = FileIntake.expand(urls)
        guard !accepted.isEmpty else { showingUnsupported = true; return }
        Task { await appState.importFiles(accepted, writer: appState.makeTempPageWriter()) }
    }
